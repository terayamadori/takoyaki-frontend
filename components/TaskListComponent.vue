    ...
    <div class="row">
      <div class="col-md-4 mb-3" v-for="order in orders" :key="order.id">
        <div class="postit-card">
          ...
          <p class="order-detail">デザート: {{ order.dessert_takoyaki_quantity }}個</p>
          <p class="order-date">受付日時: {{ formatDate(order.order_date) }}</p>
          <button class="btn btn-primary btn-lg" @click="pass(order.id, order.order_number)">受け渡し</button>
        </div>
      </div>
    </div>
  </div>
    ...
    async pass(orderId, order_number) {
      try {
        const response = await this.$axios.$patch(`/api/orders/${orderId}`, {
          status: 'completed',
          pass_date: new Date().toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' }) // 日本時間を設定
        });

        alert(`受付番号 ${order_number} の受け渡しが完了しました！`);
        this.orders = this.orders.filter(order => order.id !== orderId); // リストから削除
      } catch (error) {
        console.error('受け渡しの更新に失敗しました:', error.response ? error.response.data : error.message);
        alert('受け渡しの更新に失敗しました: ' + (error.response ? error.response.data.message : error.message));
      }
    },
    initializeSocket() {
      ...
      socket.on('orderUpdated', updatedOrder => {
        ...
        // 古い順に並べ替え
        this.orders.sort((a, b) => new Date(a.order_date) - new Date(b.order_date));
      });
      ...
      socket.on('orderDeleted', deletedOrderId => {
        this.orders = this.orders.filter(order => order.id !== deletedOrderId); // リストから削除
      });
    },
...
.order-detail,
.order-date {
  font-size: 1.0em; /* 他のテキストのフォントサイズを大きく */
}
.postit-card button {
