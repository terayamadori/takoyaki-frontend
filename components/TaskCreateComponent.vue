          ...
          <table class="table table-hover" v-if="orders.length > 0">
            <thead class="thead-light sticky-header">
              <tr>
                ...
                <th
                  scope="col"
                  @click="sortOrders('takoyaki_quantity')"
                  class="header-text"
                >
                  たこ焼き
                </th>
                <th
                  scope="col"
                  @click="sortOrders('dessert_takoyaki_quantity')"
                  class="header-text"
                >
                  デザート
                </th>
            ...
            <tbody>
              <tr v-for="order in orders" :key="order.id">
                ...
                <td class="text-center">
                  {{ order.dessert_takoyaki_quantity }}
                </td>
                <td class="text-center">
                  <span v-if="order.order_date">
                    {{
                      new Date(order.order_date).toLocaleTimeString("ja-JP", {
                        hour: "2-digit",
                        minute: "2-digit",
                        timeZone: "Asia/Tokyo",
                      })
      ...
      <div class="col-md-4">
        <form @submit.prevent="submitForm">
          <div class="form-group row">
            <label for="order_number" class="col-sm-12 col-form-label"
              >受付番号</label
            >
            <input
              type="text"
              ...
              id="order_number"
              @focus="setCurrentField('order_number')"
              required
            />
          </div>
          <div class="form-group row">
            <label for="takoyaki" class="col-sm-12 col-form-label"
              >たこ焼き: {{ existingPrices.takoyakiPrice }}円</label
            >
            <input
              type="number"
              v-model="form.takoyaki"
              class="col-sm-12 form-control"
              id="takoyaki"
              @focus="setCurrentField('takoyaki')"
              required
            />
          </div>
          <div class="form-group row">
            <label for="dessert" class="col-sm-12 col-form-label"
              >デザート: {{ existingPrices.dessertPrice }}円</label
            >
            <input
              type="number"
          ...
          <div class="calculator mt-3">
            <div class="row">
              <button
                type="button"
                class="btn btn-secondary btn-lg col-3 mx-1 my-1"
                @click="appendValue('1')"
              >
                1
              </button>
            ...
            <div class="row">
              ...
              <button
                type="button"
                class="btn btn-secondary btn-lg col-6 mx-1 my-1"
                @click="clearValue"
              >
                C
              </button>
          ...
          <button type="submit" class="btn btn-primary btn-lg col-sm-12 mt-3">
            登録
          </button>
        </form>
      </div>
  ...
  methods: {
    ...
    async fetchCurrentPrices() {
      try {
        ...
        if (response.length > 0) {
          this.existingPrices.takoyakiPrice = Math.floor(
            response[0].takoyaki_price
          );
          this.existingPrices.dessertPrice = Math.floor(
            response[0].dessert_takoyaki_price
          );
        }
      } catch (error) {
    ...
    calculateTotals() {
      ...
      this.totalDessert = this.orders
        .filter((order) => order.status === "preparing") // ステータスが preparing の注文のみをフィルタリング
        .reduce(
          (total, order) => total + (order.dessert_takoyaki_quantity || 0),
          0
        ); // 合計計算
    },
    async submitForm() {
    ...
    async deleteOrder(orderId, order_number) {
      if (confirm(`受付番号 ${order_number}を削除しますか？`)) {
        // 確認メッセージを表示
        try {
