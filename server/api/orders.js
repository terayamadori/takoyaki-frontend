...
router.get('/orders', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'データ取得に失敗しました。詳細: ' + error.message });
    }
});
...
router.get('/pricesettings', async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({ error: '価格設定データが見つかりませんでした' });
        }
        res.status(200).json(result.rows);
    } catch (error) {
        console.error('価格設定データ取得エラー:', error);
        res.status(500).json({ error: '価格設定データの取得に失敗しました。詳細: ' + error.message });
    }
});
